<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../css/style.css">
    <title>C2 CodeCamp - Latte</title>
</head>
<body class="violet-bg">
    <?php 
        require_once('../header.php');
    ?>

    <main>
    <a href="<?php echo $base_url; ?>/index.php" class="link-substructure">Home</a> > <a href="dranken.php" class="link-substructure"> Dranken</a> > <a href="#" class="link-substructure"> Warme dranken</a>  > <a href="latte.php" class="link-substructure">Latte</a> 
        <div class="wrapper">
            <div class="layout-pages">
                <div class="latte">
                    <div class="fontsize-18">
                        <h1>Latte</h1>
                        <p>Een latte macchiato is een koffiedrank die bestaat uit laagjes van melk en espresso. Latte betekent melk en macchiato staat voor gevlekt. Wat er ontstaat als er op de juiste wijze espresso aan de latte wordt toegevoegd is een vlek in de melk.</p>
                        <img src="../img/latte.png" alt="">
                    </div>
                </div>
            </div>
        </div>
    </main>
</body>
</html>