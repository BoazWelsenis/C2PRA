<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="../css/style.css">
    <title>C2 CodeCamp - Thee</title>
</head>
<body class="violet-bg">
    <?php 
        require_once('../header.php');
    ?>

    <main>
    <a href="<?php echo $base_url; ?>/index.php" class="link-substructure">Home</a> > <a href="#" class="link-substructure"> Dranken</a> > <a href="#" class="link-substructure"> Warme dranken</a>  > <a href="thee.php" class="link-substructure">Thee</a> 
        <div class="wrapper">
            <div class="layout-pages">
                <div class="thee">
                    <div class="fontsize-18">
                        <h1>Thee</h1>
                        <p>Thee is een warme drank die wordt gemaakt door infusie van de gedroogde bladeren van de theeplant Camellia sinensis. Alle als thee verkochte producten die niet van deze plant afkomstig zijn, zijn dan ook geen echte thee maar kruidenthee. De naam thee komt waarschijnlijk uit het Chinese dialect Minnanyu.</p>
                        <img src="../img/thee-2.png" alt="">
                    </div>
                </div>
            </div>
        </div>
    </main>
</body>
</html>